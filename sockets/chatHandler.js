  ...
  io.on('connection', (socket) => {
    ...
    socket.on('join_game', async (data, callback) => {
      ...
      try {
        ...
        if (!gameSummary || !gameSummary.header || !gameSummary.header.competitions) {
          ...
          socket.gameContext = { league, gameId, supportsHome, supportsAway, isNeutral };
    ...
    socket.on('vote_poll', async (data, callback) => {
      ...
      try {
        ...
        if (!msg || !msg.content.startsWith('[POLL_JSON]')) return;

        const pollData = JSON.parse(msg.content.substring(11));
        
        // Check if user already voted
        if (pollData.votedUsers.includes(socket.user.id)) {
          return callback && callback({ success: false, message: 'Already voted' });
        }

        // Add vote
        if (pollData.votes[option] !== undefined) {
          pollData.votes[option] += 1;
          pollData.votedUsers.push(socket.user.id);
        }

        const updatedMsg = await prisma.gameMessage.update({
          where: { id: messageId },
          data: { content: `[POLL_JSON]${JSON.stringify(pollData)}` },
          include: { user: { select: { id: true, username: true } } }
        });
