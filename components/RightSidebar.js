...
export default function RightSidebar() {
  ...
  const fetchMyTeams = async () => {
    try {
      const res = await fetch('http://localhost:5000/api/users/me/teams/details', {
        credentials: 'include'
      });
      if (res.ok) {
  ...
  const handleFetchAnalysis = async (leagueKey, gameId) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/sports/${leagueKey}/game/${gameId}/analysis`, {
        credentials: 'include'
      });
      if (res.ok) {
        const data = await res.json();
