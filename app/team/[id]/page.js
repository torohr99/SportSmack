...
export default function TeamPage({ params }) {
  ...
  useEffect(() => {
    const fetchTeam = async () => {
      setLoading(true);
      try {
        const res = await fetch(`http://localhost:5000/api/teams/${id}?collegeSport=${collegeSport}`, {
          credentials: 'include'
        });
        if (res.ok) {
  ...
  const toggleFollow = async () => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/teams/${id}/follow?collegeSport=${collegeSport}`, {
        method,
        credentials: 'include'
      });
      ...
      {team.stats && (
        <div className="team-stats-strip">
          <div className="stat-box">
            ...
            <div className="stat-box-value">{team.stats.wins}-{team.stats.losses}</div>
          </div>
          <div className="stat-box">
            <div className="stat-box-label">Home</div>
            <div className="stat-box-value">{team.stats.homeRecord}</div>
          </div>
          <div className="stat-box">
            <div className="stat-box-label">Away</div>
            <div className="stat-box-value">{team.stats.awayRecord}</div>
          </div>
          <div className="stat-box">
            <div className="stat-box-label">Streak</div>
            <div className="stat-box-value">{team.stats.streak}</div>
          </div>
        </div>
      )}
