...
export default function Notifications() {
  ...
  const fetchData = async () => {
    try {
      const [reqRes, notifRes] = await Promise.all([
        fetch(`http://localhost:5000/api/users/requests`, { credentials: 'include' }),
        fetch(`http://localhost:5000/api/users/me/notifications`, { credentials: 'include' })
      ]);
      ...
      if (notifRes.ok) {
        ...
        // Mark as read
        fetch(`http://localhost:5000/api/users/me/notifications/read`, { method: 'PUT', credentials: 'include' });
      }
    } catch (err) {
  ...
  const handleAction = async (userId, action) => {
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/users/${userId}/friend`, {
        method,
        credentials: 'include'
      });
