...
export default function UserProfile({ params }) {
  ...
  const fetchProfileAndFriends = async () => {
    try {
      const [profileRes, friendsRes] = await Promise.all([
        fetch(`http://localhost:5000/api/users/${id}`, { credentials: 'include' }),
        fetch(`http://localhost:5000/api/users/${id}/friends`, { credentials: 'include' })
      ]);
  ...
  const handleFriendAction = async (action) => {
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/users/${id}/friend`, {
        method,
        credentials: 'include'
      });
