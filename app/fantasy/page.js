...
export default function FantasyDashboard() {
  ...
  const fetchLeagues = async () => {
    try {
      const res = await axios.get('http://localhost:5000/api/fantasy/leagues', { withCredentials: true });
      setLeagues(res.data);
    } catch (err) {
  ...
  const createLeague = async (e) => {
    ...
    try {
      await axios.post('http://localhost:5000/api/fantasy/league', { name: newLeagueName }, { withCredentials: true });
      setNewLeagueName('');
      fetchLeagues();
    } catch (err) {
  ...
  const joinLeague = async (e) => {
    ...
    try {
      await axios.post(`http://localhost:5000/api/fantasy/league/${joinLeagueId}/join`, { teamName: joinTeamName }, { withCredentials: true });
      setJoinLeagueId('');
      setJoinTeamName('');
      fetchLeagues();
    } catch (err) {
  ...
  const seedPlayers = async () => {
    try {
      ...
      const res = await axios.post('http://localhost:5000/api/fantasy/seed', {}, { withCredentials: true });
