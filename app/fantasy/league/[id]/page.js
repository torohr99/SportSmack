...
export default function LeaguePage({ params }) {
  ...
  const fetchLeague = async () => {
    try {
      const res = await axios.get(`http://localhost:5000/api/fantasy/league/${id}`, { withCredentials: true });
      setLeague(res.data);
    } catch (err) {
  ...
  const setRosterStatus = async (teamId, teamPlayerId, status) => {
    try {
      await axios.post(`http://localhost:5000/api/fantasy/team/${teamId}/roster`, { teamPlayerId, status }, { withCredentials: true });
      fetchLeague(); // Refresh
    } catch (err) {
